...
```
### Observations
Time Series dataset is missing some data that requires filling in. 
To create these charts, missing weeks were arbitrarily filled in with "0". 

The Remainder chart does not appear to be random. This would need to be explored
further to determine if there are any uncaptured season trends, or autocorrelations. 
...
Based on average wait time, this does not have any seasonality or cyclic elements. 
Some weeks were missing data and therefore arbitrarily imputed with the mean wait time. 
