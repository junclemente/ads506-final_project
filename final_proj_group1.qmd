...
The outcome of the Augmented Dickey-Fuller indicates that we are unable to reject the null hypothesis that the series is non-stationary as the p-value is \> 0.05 with a value of 0.2735.
...
snaive_model <- tng_df |>
  model(SNAIVE(differenced_calls ~ lag(7)))
...
AA_temp_model <- tng_df |>
  model(ARIMA(differenced_calls ~ PDQ(0,0,0) + 
                TMAX_CEL))
...
SAA_temp_model <- tng_df |>
  model(ARIMA(differenced_calls ~ TMAX_CEL))
...
### Seasonal Auto ARIMA (5,0,0)(1,0,0) Applied to Entire Series

```{r}
# refit pre-trained SAA model to entire dataset
SAA_refit <- SAA_Model |>
  refit(model_ts_diff)

# forecast the next 31 days
SAA_final_forecast <- SAA_refit |>
  forecast(h = 31)

# Visualize the forecast
SAA_final_plot <- autoplot(SAA_final_forecast, model_ts_diff) +
  ggtitle("31-Day Forecast using Refitted Seasonal Auto ARIMA") +
  labs(y = "Differenced Calls", x = "Date")
...
```{r}
# reverse differencing to get forecast in the original scale
last_observed <- model_ts$total_calls[nrow(model_ts)]

SAA_original_scale <- SAA_final_forecast |>
  mutate(forecast_calls = .mean + last_observed)

# view the original-scale forecast
# print(SAA_original_scale)
...
nov_df <- read.csv("C:/Users/graha/Documents/MS Applied Data Science/ADS_506_TimeSeries/Project/ads506-final_project/Nov_edify_calls.csv")
...
# merge
nov_results <- full_join(nov_daily_counts, SAA_original_scale, by = "Date")

# drop unnecessary columns
nov_results <- nov_results |> 
  select(-.model, -differenced_calls, -.mean) |>
  mutate(forecast_calls = as.integer(forecast_calls)) |> 
  mutate(Error = forecast_calls - nov_actual)
...
```{r}
# original counts df with temperature variable dropped
original_dc <- model_df |> 
  select(-TMAX_CEL, -Day)
...
final_viz <- ggplot() +
  ...
  geom_line(data = nov_results,
            ...
            color = "red",
            linetype = "dashed") + 
  labs(title = "SAA Time Series Monthly Forecast for November",
       x = "Date",
       y = "Calls")
